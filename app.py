...
with tab1:
    ...
    # 1. ดึงข้อมูลบ่อสี พร้อมกับ color_id (ต้องแน่ใจว่าในตาราง tanks มีคอลัมน์ color_id)
    # เราใช้ .select("*") หรือระบุคอลัมน์ที่จำเป็น เพื่อให้ได้ข้อมูลครบ
    try:
        response = supabase.table("tanks").select("tank_id, tank_name, color_id").eq("tank_type", "Color").execute()
        tanks_list = response.data # รายการบ่อทั้งหมด
    except:
        tanks_list = []

    # ดึงรายการสีทั้งหมด (เพื่อแสดงใน Dropdown)
    color_options_dict = get_options("colors", "color_id", "color_name") # {color_name: color_id}
    color_names = list(color_options_dict.keys())
    # สร้าง Map สำหรับหาชื่อสีจาก ID: {color_id: color_name}
    id_to_color_name = {v: k for k, v in color_options_dict.items()}

    if tanks_list and color_options_dict:
        # สร้างรายการชื่อบ่อสำหรับ Dropdown
        tank_names = [t['tank_name'] for t in tanks_list]
        
        col_tank, col_color = st.columns(2)
        
        with col_tank:
            selected_tank_name = st.selectbox("เลือกบ่อสี", tank_names, key="tank_select_t1")
        
        # หาข้อมูลบ่อที่ถูกเลือก
        selected_tank_data = next((t for t in tanks_list if t['tank_name'] == selected_tank_name), None)
        
        # หาชื่อสีเริ่มต้นจาก color_id ของบ่อ
        default_color_name = id_to_color_name.get(selected_tank_data.get('color_id'), color_names[0])
        
        # หา index ของสีในรายการ Dropdown เพื่อให้มันเลือกให้อัตโนมัติ
        try:
            default_index = color_names.index(default_color_name)
        except ValueError:
            default_index = 0

        with col_color:
            selected_color = st.selectbox("เลือกสีในบ่อ", color_names, index=default_index, key="color_select_t1")
        
        # แสดงแถบสีตามการเลือก
        render_color_bar(selected_color)
            ...
            if st.form_submit_button("บันทึกค่ามาตรฐาน"):
                if ph == 0 or temp == 0:
                    ...
                    supabase.table("color_tank_logs").insert({
                        "tank_id": selected_tank_data['tank_id'], 
                        "ph_value": ph, 
                        "temperature": temp,
        ...
        st.warning("ไม่พบข้อมูลบ่อสีหรือรายการสีในระบบ")
