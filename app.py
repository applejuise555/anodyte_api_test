...
with tab1:
    ...
    # 1. ดึงข้อมูลบ่อสีทั้งหมดจากฐานข้อมูล (รวมถึงข้อมูลสีที่ผูกกับบ่อ)
    # หมายเหตุ: ในตาราง tanks ของคุณต้องมีคอลัมน์ color_name หรือข้อมูลสีที่เชื่อมโยงกันอยู่
    try:
        # ปรับ query ให้ดึง color_name มาด้วย (ปรับชื่อคอลัมน์ตามจริงใน Database ของคุณ)
        response = supabase.table("tanks").select("tank_id, tank_name, color_name").eq("tank_type", "Color").execute()
        tanks_data = response.data
    except Exception as e:
        tanks_data = []
        st.error(f"ไม่สามารถดึงข้อมูลบ่อสีได้: {e}")

    if tanks_data:
        # สร้างรายการชื่อบ่อสำหรับ Selectbox
        tank_options = {item['tank_name']: item for item in tanks_data}
        
        selected_tank_name = st.selectbox("เลือกบ่อสี", list(tank_options.keys()), key="tank_select_t1")
        
        # ดึงข้อมูลของบ่อที่เลือก
        selected_tank = tank_options[selected_tank_name]
        detected_color = selected_tank.get('color_name', 'Black') # ดึงสีที่ผูกกับบ่อมา
        
        # แสดงผลสีที่ตรวจพบ
        st.write(f"สีที่ใช้ในบ่อนี้: **{detected_color}**")
        render_color_bar(detected_color) # แสดงแถบสีอัตโนมัติ
