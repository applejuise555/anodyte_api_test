...
with tab1:
    ...
    colors = get_options("colors", "color_id", "color_name") # ดึงรายการสีมาด้วย
    
    if color_tanks and colors:
        col_tank, col_color = st.columns(2)
        with col_tank:
            selected_tank = st.selectbox("เลือกบ่อสี", list(color_tanks.keys()), key="tank_select_t1")
        with col_color:
            selected_color = st.selectbox("เลือกสีในบ่อ", list(colors.keys()), key="color_select_t1")
        
        # แสดงแถบสีตามการเลือกสี (เหมือนกับ Tab 3)
