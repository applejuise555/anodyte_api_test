...
with tab1:
    ...
    if color_tanks:
        ...
        # 1. บันทึกข้อมูลมาตรฐาน (color_tank_logs)
        with st.form("color_log_form", clear_on_submit=True):
            st.subheader("บันทึกค่า pH และอุณหภูมิ")
            ph = st.number_input("ค่า pH", step=0.1)
            temp = st.number_input("อุณหภูมิ (°C)", step=0.1)
            if st.form_submit_button("บันทึกข้อมูลปกติ"):
                ...
                st.success("บันทึกข้อมูลปกติสำเร็จ!")

        # 2. บันทึกอุณหภูมิความถี่สูง (temp_frequent_logs)
        with st.expander("บันทึกอุณหภูมิราย 10 นาที (High Frequency)"):
            with st.form("freq_temp_form", clear_on_submit=True):
                target_temp = st.number_input("อุณหภูมิเป้าหมาย (Target)", step=0.1)
                actual_temp = st.number_input("อุณหภูมิที่วัดได้จริง (Actual)", step=0.1)
                if st.form_submit_button("บันทึกค่าอุณหภูมิ"):
                    ...
                    st.success("บันทึกค่าอุณหภูมิความถี่สูงสำเร็จ!")
